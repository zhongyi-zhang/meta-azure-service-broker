...
module.exports.getConfigurations = function() {
  ...
  var config = {
    ...
    'privilege': {
        'sqldb': {
            'allowToCreateSqlServer': process.env['ALLOW_TO_CREATE_SQL_SERVER'],
        }
    },
    'accountPool': {
        'sqldb': {
  ...
  if (process.env['SQL_SERVER_POOL']) {
          
      var sqlServerPool = config['accountPool']['sqldb'];
      try {
          var sqlServers = JSON.parse(process.env['SQL_SERVER_POOL']);
          sqlServers.forEach(function(sqlServer){
              sqlServerPool[sqlServer['sqlServerName']] = {
                  administratorLogin: sqlServer['ADMINISTRATOR_LOGIN'],
                  administratorLoginPassword: sqlServer['ADMINISTRATOR_LOGIN_PASSWORD']
              };
          });
      } catch (e) {
          console.error('format error in manifest parameter: SQL_SERVER_POOL, err: %j', e);
      }
  }
