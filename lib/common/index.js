...
module.exports.getConfigurations = function() {
  ...
  var config = {
    ...
    'privilege': {
        'sqldb': {
            'isAllowedToCreateSqlServer': process.env['IS_ALLOWED_TO_CREATE_SQL_SERVER'],
            'isAllowedToConfigureFirewallRules': process.env['IS_ALLOWED_TO_CONFIGURE_FIREWALL_RULES']
        }
    },
    'accountPool': {
        'sqldb': {
  ...
  for (var moduleName in config['privilege']) {
      var module = config['privilege'][moduleName];
      for (var opt in module) {
          if (!module[opt]) module[opt] = defaultPrivilege[moduleName][opt];
      }
  }
  
  // import admin accounts TODO: import a list of accounts
  if (process.env['SQL_SERVER_NAME'] &&
      process.env['ADMINISTRATOR_LOGIN'] &&
      process.env['ADMINISTRATOR_LOGIN_PASSWORD']) {
          
      var sqlPool = config['accountPool']['sqldb'];
      sqlPool[process.env['SQL_SERVER_NAME']] = {
          administratorLogin: process.env['ADMINISTRATOR_LOGIN'],
          administratorLoginPassword: process.env['ADMINISTRATOR_LOGIN_PASSWORD']
      };
  }
