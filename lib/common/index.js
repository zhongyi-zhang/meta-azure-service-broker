...
var validateConfigurations = function() {
  ...
  function validateAccountPool(servers, params) {
    servers.forEach(function(server){
               
      params.forEach(function(param){
        if (!server[param]) {
          throw new Error(util.format('Please check %j in your broker configuration.', server));
        }
      });
                 
      // pcf-tile pass the value of administratorLoginPassword as {"secret": "<password>"}
      if (typeof server['administratorLoginPassword'] === 'object') {
        server['administratorLoginPassword'] = server['administratorLoginPassword']['secret'];
      }
                
      params.forEach(function(param){
        if (typeof server[param] !== 'string') {
          throw new Error(util.format('Please check %j in your broker configuration.', server));
        }
      });
    });
  ...
  if (process.env['AZURE_SQLDB_SQL_SERVER_POOL']) {
    ...
    validateAccountPool(sqlServers, params);
  }
  
  // validate mysql server 
  if (process.env['AZURE_MYSQLDB_MYSQL_SERVER_POOL']) {
    var mysqlServers = JSON.parse(process.env['AZURE_MYSQLDB_MYSQL_SERVER_POOL']);
    var params = ['resourceGroup', 'location', 'mysqlServerName', 'administratorLogin', 'administratorLoginPassword'];
    validateAccountPool(mysqlServers, params);
  }
};
...
module.exports.getConfigurations = function() {
  ...
  var config = {
    ...
    'privilege': {
      ...
      'mysqldb': {
        'allowToCreateMysqlServer': (process.env['AZURE_MYSQLDB_ALLOW_TO_CREATE_MYSQL_SERVER'] === undefined || process.env['AZURE_MYSQLDB_ALLOW_TO_CREATE_MYSQL_SERVER'] === 'true')
      }
    },
    'accountPool': {
      'sqldb': {
  ...
  function importAccountPool(serverPool, servers, params, key) {
    servers.forEach(function(server){
      ...
      serverPool[server[key]] = {};  
      params.forEach(function(param){
        serverPool[server[key]][param] = server[param];
      });
    });
  }
  ...
  if (process.env['AZURE_SQLDB_SQL_SERVER_POOL']) {
    ...
    var sqlServers = JSON.parse(process.env['AZURE_SQLDB_SQL_SERVER_POOL']);
    var params = ['resourceGroup', 'location', 'administratorLogin', 'administratorLoginPassword'];
    importAccountPool(sqlServerPool, sqlServers, params, 'sqlServerName');
  }
  
  // import mysql admin accounts
  if (process.env['AZURE_MYSQLDB_MYSQL_SERVER_POOL']) {
    var mysqlServerPool = config['accountPool']['mysqldb'];
    var mysqlServers = JSON.parse(process.env['AZURE_MYSQLDB_MYSQL_SERVER_POOL']);
    var params = ['resourceGroup', 'location', 'administratorLogin', 'administratorLoginPassword'];
    importAccountPool(mysqlServerPool, mysqlServers, params, 'mysqlServerName');
  }
