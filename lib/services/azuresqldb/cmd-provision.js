...
var sqldbProvision = function (log, params) {
    ...
    this.provision = function (sqldbOperations, resourceGroup, next) {
        ...
        var fullyQualifiedDomainName, administratorLogin, administratorLoginPassword;
        ...
        async.waterfall([
            ...
            function (result, callback) {   // create sql server if not exist
                if (result.statusCode === HttpStatus.NOT_FOUND) {
                    if (!privilege.isAllowedToCreateSqlServer) {
                        var errorMessage = util.format('The SQL server %s doesn\'t exist and you don\'t have the privilege to create a new server.', sqlServerName);
                        return common.handleServiceError(log, Error(errorMessage), callback);
                    }
                    ...
                    if (!(administratorLogin && administratorLoginPassword)) {
                        if (sqlServerName in accountPool) {
                            ...
                            var errorMessage = util.format('Can\'t find administratorLogin or administratorLoginPassword for the SQL server %s.', sqlServerName);
                            return common.handleServiceError(log, Error(errorMessage), callback);
                        }
                    }
            ...
            function (callback) {  // open firewall IP if requested
                if (firewallRules !== null) {
                    if (!privilege.isAllowedToConfigureFirewallRules) {
                        var errorMessage = util.format('You don\'t have the privilege to create a new server.', sqlServerName);
                        return common.handleServiceError(log, Error(errorMessage), callback);
                    }
