...
Handlers.provision = function(log, params, next) {
  ...
  async.waterfall([
    function(callback) {
      utils.checkNamespaceAvailability(azure_config, function(err) {
        callback(err);
      });
    },
    function(callback) {
      utils.createResourceGroup(azure_config, function(err) {
        callback(err);
      });
    },
    function(callback) {
      utils.createNamespace(azure_config, function(err) {
        callback(err);
      });
    }
  ], function(err) {
